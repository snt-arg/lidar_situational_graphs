...
private:
    ...
    for(const auto& room_data_msg : room_data_queue) {
      for(const auto& room_data : room_data_msg.rooms) {
        ...
        if(room_data.x_planes.size() == 2 && room_data.y_planes.size() == 2) {
          ...
          finite_room_mapper->factor_rooms(graph_slam, x_planes_room, y_planes_room, x_vert_planes, y_vert_planes, dupl_x_vert_planes, dupl_y_vert_planes, rooms_vec);
        }
        // x corridor
  ...
  void map_room_from_existing_corridors(const s_graphs::RoomData& det_room_data, const s_graphs::Corridors& matched_x_corridor, const s_graphs::Corridors& matched_y_corridor) {
    ...
    room_data_association = finite_room_mapper->associate_rooms(room_pose, rooms_vec);
