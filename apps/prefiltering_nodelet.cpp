...
public:
  ...
  PrefilteringNode() : Node("prefiltering_node") {
    ...
    if(this->get_parameter("deskewing").get_parameter_value().get<bool>()) {
      imu_sub = this->create_subscription<sensor_msgs::msg::Imu>("/imu/data", 1, std::bind(&PrefilteringNode::imu_callback, this, std::placeholders::_1));
    }
...
private:
  void initialize_params() {
    ...
    std::string outlier_removal_method = this->get_parameter("outlier_removal_method").get_parameter_value().get<std::string>();
    if(outlier_removal_method == "STATISTICAL") {
      int mean_k = this->get_parameter("outlier_removal_method").get_parameter_value().get<int>();
      double stddev_mul_thresh = this->get_parameter("statistical_stddev").get_parameter_value().get<int>();
  ...
  pcl::PointCloud<PointT>::Ptr deskewing(const pcl::PointCloud<PointT>::Ptr& cloud) {
    ...
    double scan_period = this->get_parameter("scan_period").get_parameter_value().get<double>();
    for(size_t i = 0; i < cloud->size(); i++) {
