...
TEST(testPlane, ConvertPlaneToMap) {
  ...
  s_graphs::KeyFrame::Ptr keyframe(new s_graphs::KeyFrame(ros::Time::now(), odom, 0.0, cloud));
  keyframe->node = graph_slam.add_se3_node(odom);
  Eigen::Vector4d local_plane;
  local_plane << 1, 0, 0, 10;
  g2o::Plane3D det_plane_body_frame(local_plane);
  g2o::Plane3D det_plane_map_frame = plane_mapper.convert_plane_to_map_frame(keyframe, local_plane);
