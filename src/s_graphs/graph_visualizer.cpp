...
GraphVisualizer::GraphVisualizer(const rclcpp::Node::SharedPtr node) {
  map_frame_id = node->get_parameter("map_frame_id").get_parameter_value().get<std::string>();
...
visualization_msgs::msg::MarkerArray GraphVisualizer::create_marker_array(const rclcpp::Time& stamp, const g2o::SparseOptimizer* local_graph, const std::vector<VerticalPlanes>& x_plane_snapshot, const std::vector<VerticalPlanes>& y_plane_snapshot, const std::vector<HorizontalPlanes>& hort_plane_snapshot, std::vector<InfiniteRooms> x_infinite_room_snapshot, std::vector<InfiniteRooms> y_infinite_room_snapshot, std::vector<Rooms> room_snapshot, double loop_detector_radius, std::vector<KeyFrame::Ptr> keyframes, std::vector<Floors> floors_vec) {
  ...
  for(int i = 0; i < x_infinite_room_snapshot.size(); ++i) {
    ...
    if(!overlapped_infinite_room) {
      ...
      corr_x_line_marker.lifetime = rclcpp::Duration(15.0);
    } else {
    ...
    if(!overlapped_infinite_room) {
      ...
      infinite_room_pose_marker.lifetime = rclcpp::Duration(15.0);
  ...
  for(int i = 0; i < y_infinite_room_snapshot.size(); ++i) {
    ...
    if(!overlapped_infinite_room) {
      ...
      corr_y_line_marker.lifetime = rclcpp::Duration(15.0);
    } else {
  ...
  room_marker.lifetime = rclcpp::Duration(15.0);
  ...
  for(int i = 0; i < room_snapshot.size(); ++i) {
    ...
    room_line_marker.lifetime = rclcpp::Duration(15.0);
  ...
  x_corr_neighbour_line_marker.lifetime = rclcpp::Duration(15.0);
  ...
  y_corr_neighbour_line_marker.lifetime = rclcpp::Duration(15.0);
  ...
  room_neighbour_line_marker.lifetime = rclcpp::Duration(15.0);
  ...
  for(const auto& floor : floors_vec) {
    if(floor.id != -1) {
      ...
      floor_marker.lifetime = rclcpp::Duration(10.0);
      ...
      floor_line_marker.lifetime = rclcpp::Duration(10.0);
