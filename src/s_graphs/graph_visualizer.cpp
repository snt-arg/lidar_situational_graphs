  ...
  for(const auto& x_corridor : x_corridor_snapshot) {
    for(const auto& x_corridor_neighbour_id : x_corridor.neighbour_ids) {
      ...
      if(found_neighbour_room != room_snapshot.end()) {
        ...
        auto found_neighbour_x_corr = std::find_if(x_corridor_snapshot.begin(), x_corridor_snapshot.end(), boost::bind(&InfiniteRooms::id, _1) == x_corridor_neighbour_id);
        if(found_neighbour_x_corr != x_corridor_snapshot.end()) {
          ...
          auto found_neighbour_y_corr = std::find_if(y_corridor_snapshot.begin(), y_corridor_snapshot.end(), boost::bind(&InfiniteRooms::id, _1) == x_corridor_neighbour_id);
  ...
  for(const auto& y_corridor : y_corridor_snapshot) {
    for(const auto& y_corridor_neighbour_id : y_corridor.neighbour_ids) {
      ...
      if(found_neighbour_room != room_snapshot.end()) {
        ...
        auto found_neighbour_x_corr = std::find_if(x_corridor_snapshot.begin(), x_corridor_snapshot.end(), boost::bind(&InfiniteRooms::id, _1) == y_corridor_neighbour_id);
        if(found_neighbour_x_corr != x_corridor_snapshot.end()) {
          p2.x = (*found_neighbour_x_corr).node->estimate()(0);
          p2.y = (*found_neighbour_x_corr).node->estimate()(1);
          p2.z = corridor_node_h;
          ...
          auto found_neighbour_y_corr = std::find_if(y_corridor_snapshot.begin(), y_corridor_snapshot.end(), boost::bind(&InfiniteRooms::id, _1) == y_corridor_neighbour_id);
          if(found_neighbour_y_corr != y_corridor_snapshot.end()) {
            p2.x = (*found_neighbour_y_corr).node->estimate()(0);
            p2.y = (*found_neighbour_y_corr).node->estimate()(1);
            p2.z = corridor_node_h;
  ...
  for(const auto& room : room_snapshot) {
    for(const auto& room_neighbour_id : room.neighbour_ids) {
      ...
      if(found_neighbour_room != room_snapshot.end()) {
        ...
        auto found_neighbour_x_corr = std::find_if(x_corridor_snapshot.begin(), x_corridor_snapshot.end(), boost::bind(&InfiniteRooms::id, _1) == room_neighbour_id);
        if(found_neighbour_x_corr != x_corridor_snapshot.end()) {
          p2.x = (*found_neighbour_x_corr).node->estimate()(0);
          p2.y = (*found_neighbour_x_corr).node->estimate()(1);
          p2.z = corridor_node_h;
          ...
          auto found_neighbour_y_corr = std::find_if(y_corridor_snapshot.begin(), y_corridor_snapshot.end(), boost::bind(&InfiniteRooms::id, _1) == room_neighbour_id);
          if(found_neighbour_y_corr != y_corridor_snapshot.end()) {
            p2.x = (*found_neighbour_y_corr).node->estimate()(0);
            p2.y = (*found_neighbour_y_corr).node->estimate()(1);
            p2.z = corridor_node_h;
